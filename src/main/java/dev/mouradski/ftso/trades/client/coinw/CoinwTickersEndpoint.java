...
@ApplicationScoped
public class CoinwTickersEndpoint extends AbstractClientEndpoint implements HttpTickers {
            ...
            try {
                ...
                tickerResponse.getData().entrySet().forEach(e -> {
                    ...
                    if (getAssets(true).contains(pair.getLeft()) && getAllQuotesExceptBusd(true).contains(pair.getRight())) {
                        var ticker = Ticker.builder().exchange(getExchange()).base(pair.getLeft()).quote(pair.getRight()).lastPrice(Double.valueOf(e.getValue().getLast())).timestamp(currentTimestamp()).build();
                        pushTicker(ticker);
                    }
                });
